                                  ...
                                  (vec
                                    (conj
                                      ...
                                      (rj.c/map->Entity {:id   e-snake
                                                         :type :snake})))))
         (rj.e/add-c e-snake (rj.c/map->Snake {}))
         (rj.e/add-c e-snake (rj.c/map->Position {:x    (:x target-tile)
                                                     :y    (:y target-tile)
                                                     :type :snake}))
         (rj.e/add-c e-snake (rj.c/map->Mobile {:can-move?-fn rj.m/can-move?
                                                   :move-fn      rj.m/move}))
         (rj.e/add-c e-snake (rj.c/map->Sight {:distance 4}))
         (rj.e/add-c e-snake (rj.c/map->Attacker {:atk              1
                                                     :can-attack?-fn   rj.atk/can-attack?
                                                     :attack-fn        rj.atk/attack
                                                     :is-valid-target? (fn [type]
                                                                         (#{:player} type))}))
         (rj.e/add-c e-snake (rj.c/map->Destructible {:hp             3
                                                         :defense        1
                                                         :can-retaliate? false
                                                         :take-damage-fn rj.d/take-damage}))
         (rj.e/add-c e-snake (rj.c/map->Killable {:experience 1}))
         (rj.e/add-c e-snake (rj.c/map->Tickable {:tick-fn process-input-tick
                                                     :pri 0}))
         (rj.e/add-c e-snake (rj.c/map->Broadcaster {:msg-fn (constantly "the snake")}))))))
